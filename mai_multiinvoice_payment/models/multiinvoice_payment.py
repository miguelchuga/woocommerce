...
class AccountPayment(models.Model):
    ...
    def _onchange_to_get_vendor_invoices(self):
        if self.payment_type in ['inbound', 'outbound'] and self.partner_type and self.partner_id and self.currency_id:
            ...
            invoice_recs = self.env['account.move'].search([('partner_id', 'child_of', self.partner_id.id), ('state', '=', 'posted'), ('type', '=', invoice_type), ('invoice_payment_state', '!=', 'paid'), ('currency_id', '=', self.currency_id.id)])
